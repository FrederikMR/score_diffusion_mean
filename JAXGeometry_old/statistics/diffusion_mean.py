...
def initialize(M):
    ...
    # guide function
    phi = lambda q,v,s: jnp.tensordot((1/s)*jnp.linalg.cholesky(M.g(q)).T,M.Log(q,M.F((v,q[1]))).flatten(),(1,0))
    ...
    M.diffusion_mean_likelihood = lambda samples, x, N=N: neg_log_p_Ts(x[0],x[1],samples,dWs(len(samples[0])*N*M.dim,_dts).reshape(-1,_dts.shape[0],N,M.dim),_dts,jnp.array(.2,dtype="float32"))
