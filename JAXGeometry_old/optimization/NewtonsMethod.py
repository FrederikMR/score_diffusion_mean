...
@jit
def RMNewtonsMethod(mu_init:jnp.ndarray,
                    M:object,
    ...
    @jit
    def update(carry:tuple[jnp.ndarray, jnp.ndarray], idx:int
...
@jit
def NewtonsMethod(mu_init:jnp.ndarray,
                  grad_fn:Callable[[jnp.ndarray], jnp.ndarray],
                  ggrad_fn:Callable[[jnp.ndarray], jnp.ndarray] = None,
                  step_size:float = 0.1,
                  max_iter:int=100,
...
@jit 
def JointNewtonsMethod(mu_rm:jnp.ndarray,
                       mu_euc:jnp.ndarray,
                       M:object,
