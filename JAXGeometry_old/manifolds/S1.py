...
from JAXGeometry.manifolds.riemannian import metric, geodesic, Log, parallel_transport
...
class Ellipsoid(EmbeddedManifold):
    ...
    def __init__(self, 
        ...
        do_chart_update_spherical = lambda x: True #(x[0] > self.angle_shift-.1) or (x[0] < self.angle_shift-2*jnp.pi-.1) 
        ...
        metric.initialize(self)
        geodesic.initialize(self)
        Log.initialize(self)
        parallel_transport.initialize(self)
        
        if use_spherical_coords:
            self.Exp = self.StdExpSpherical
    ...
    def StdExpSpherical(self, x, v):
        
        return (x[0]+v[0]) % self.angle_shift
