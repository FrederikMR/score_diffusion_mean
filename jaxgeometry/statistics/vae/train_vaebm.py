...
from jaxgeometry.statistics.score_matching.loss_fun import *
...
def train_vaebm(vae_model:object,
    ...
    def vae_loss_grad(params:hk.Params, state_val:dict, rng_key:Array, x:Array):
        ...
        s_logqzx = score_apply_fn(score_state.params, jnp.hstack((mu,z,t)), 
                                  score_state.rng_key, score_state.state_val)
        s_logpz = score_apply_fn(score_state.params, jnp.hstack((mu0,z,t0*jnp.ones_like(t))), 
                                 rng_key, state.state_val)
    ...
    if type(vae_model) == hk.Transformed:
        if vae_state is None:
            initial_params = vae_model.init(jrandom.PRNGKey(seed), next(vae_dataset).x)
        ...
        if vae_state is None:
            initial_params, init_state = vae_model.init(jrandom.PRNGKey(seed), next(vae_dataset).x)
            initial_opt_state = optimizer.init(initial_params)
    ...
    for step in range(burnin_epochs):
        for i in range(10):
            ...
            vae_state = update_vae_fun(vae_state, ds.x)
    ...
    for step in range(joint_epochs):
        for i in range(repeats):
            ds = next(vae_datasets)
            vae_state = update_vae_grad(vae_state, ds.x)
        F = lambda z: decoder_apply_fn(vae_state.params, z[0].reshape(-1,2), vae_state.rng_key, 
                                       vae_state.state_val)[0].reshape(-1)
        M = Latent(dim=dim, emb_dim=emb_dim, F = F)
        x0 = (jnp.zeros(dim), jnp.zeros(1))
