...
def trainxt(manifold:str="RN",
    ...
    if manifold == "RN":
        ...
        vae_state = load_model('ManLearn/models/MNIST/VAE/')
        F = lambda x: mnist_decoder.apply(vae_state.params, vae_state.rng_key, x[0]).reshape(-1)
        
        M = Latent(F=F,dim=2,emb_dim=28*28,invF=None)
        Brownian_coords(M)
        
        N_dim = M.dim
        x0 = mnist_encoder.apply(vae_state.params, vae_state.rng_key, next(ds).image)
        x0 = M.coords(x0[0])
        ...
        state = load_model(''.join(('scores/MNIST/',loss_type,'/')))
