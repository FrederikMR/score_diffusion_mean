...
def parse_args():
    ...
    # File-paths
    parser.add_argument('--save_path', default="ManLearn/models/MNIST/VAE/",
                        type=str)
    ...
    parser.add_argument('--seed', default=2712, 
                        type=int)
    parser.add_argument('--lr_rate', default=0.0005,
                        type=float)
...
def train_vae():
    ...
    def loss_fn(params, rng_key, batch:Batch)->Array:
        ...
        out, _ = model.apply(params, state.state_val, rng_key, batch.image)
        z, x_hat, mu, std = out
    ...
    def update(state: TrainingState, batch: Batch) -> TrainingState:
      ...
      return TrainingState(new_params, state.state_val, new_opt_state, next_rng_key), aux
    
    # Load datasets.
    train_dataset, N_data = load_dataset("train", 64, 2712)
    optimizer = optax.adam(args.lr_rate)

    
    # Initialise the training state.
    initial_rng_key = jran.PRNGKey(args.seed)
    initial_params, init_state = model.init(initial_rng_key, next(train_dataset).image)
